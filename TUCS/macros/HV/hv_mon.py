...
if mode=='RegularJob':		
	ini_Date = datetime.date.today() - datetime.timedelta(days=32) # ini_date (yesterday's date - 31 days)	
	fin_Date = datetime.date.today() - timedelta(days=1) # fin_date (yesterday's date) 
...
#
processors.append( Listmaker(ini_Date, fin_Date, mode) )

#
processors.append( dataMerger(ini_Date, fin_Date, mode) )

#
processors.append( dataAnalyser(ini_Date, fin_Date, mode) )
# Go!
Go(processors)

#fs setacl Or/2013/03 -acl sromanos all
