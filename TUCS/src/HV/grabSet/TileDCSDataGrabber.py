...
#            self.db = ROOT.TSQLServer.Connect("oracle://pcata0022.cern.ch/ATONR_PVSSPROD","ATLAS_PVSS_READER","PVSSRED4PRO")
            ...
            raise "Unknown dbSource: \"%s\", please use either COOL or ORACLE"
    ...
    def getDCSTree(self, iovStartUnix, iovEndUnix, drawers, variables):
        ...
        print("into getDCSTree(self", iovStartUnix, iovEndUnix, drawers, variables, ")")
        ...
        t.SetDirectory(0)
        evTime = array('l',[0])
        t.Branch('EvTime.order', evTime, 'EvTime.order/I')
        weight = array('f',[0])
        t.Branch('weight.order', weight, 'weight.order/F')
        buffer = {}
        for drawer, variable in uniqueVariables:
            ...
            t.Branch( varName+".order" , buffer[varName], varName+".order"+treeType )
    ...
    def getEventHistoryTables(self, iovStart, iovEnd):
        ...
        if int(iovStart) < oldTableBoundary:
            if int(iovEnd) > oldTableBoundary:
                raise ("Can not generate plots of table boundary at 06/25/2007, 15:06:48, sorry...")
