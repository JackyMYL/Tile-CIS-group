############################################################
#
# do_pmts_plot_compare_TimeEvolution.py
#
############################################################
#
...
class do_pmts_plot_compare_TimeEvolution(GenericWorker):
                        ...
                        if 'pmt_ratio' in event.data:
                            if 'deviation' in event.data:
                                if (event.data['pmt_ratioref']!=0):
                                    y = 100*(event.data['pmt_ratio']-event.data['pmt_ratioref'])/event.data['pmt_ratioref']-event.data['deviation']
                                    ey = 0
                                    n = tgraph.GetN()
...
#
    ...
    def DoPerDrawerPlots(self, i_part, i_drawer, pmt_cell_name):
        ...
        for i_pmt in range(48):
            ...
            hist.GetYaxis().SetTitle("Difference variation [%]")
        ...
        self.c1.Update()
        # raw_input()
        plot_name = "time_evolution_difference_%s_history" % (self.PMTool.get_module_name(i_part,i_drawer))
