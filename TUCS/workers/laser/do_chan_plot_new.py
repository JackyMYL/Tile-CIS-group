...
class do_chan_plot_new(GenericWorker):
...
#                                      100, 0, self.time_max-self.time_min+1,-10000, 10000)
        ...
        tgraph_deviation          = [ROOT.TGraphErrors(), ROOT.TGraphErrors()]
        tgraph_deviation_bad      = [ROOT.TGraphErrors(), ROOT.TGraphErrors()]
        tgraph_deviation_affected = [ROOT.TGraphErrors(), ROOT.TGraphErrors()]

        tgraph_signal   = [ROOT.TGraphErrors(), ROOT.TGraphErrors()]
        tgraph_bad      = [ROOT.TGraphErrors(), ROOT.TGraphErrors()]
        tgraph_affected = [ROOT.TGraphErrors(), ROOT.TGraphErrors()]
        tgraph_saturated = [ROOT.TGraphErrors(), ROOT.TGraphErrors()]

        tgraph_hvabs = ROOT.TGraph()
        tgraph_hvcor = ROOT.TGraph()

        tgraph_flaser = ROOT.TGraph()


        ROOT.gStyle.SetTimeOffset(self.origin.Convert())

        self.cadreSignal.GetXaxis().SetTimeDisplay(1)
        self.cadreSignal.GetYaxis().SetTitleOffset(.7)
        ...
        self.cadreDeviation.GetYaxis().SetTitleOffset(.7)
        ...
        for gain in [0, 1]:
            for event in events[gain]: # fill the histogram
                ...
                if (event.data['status']&0x4) or (event.data['status']&0x10):
                    tgraph = tgraph_bad[gain]
                elif event.data['status']&0x100:
                    tgraph = tgraph_saturated[gain]
                elif event.data['status']!=0:
                    tgraph = tgraph_affected[gain]
                else:
                    tgraph = tgraph_signal[gain]

                npoints = tgraph.GetN()
                tgraph.SetPoint(npoints, event.run.time_in_seconds-self.time_min, corr[gain]*event.data['signal'])
                tgraph.SetPointError(npoints, 0, corr[gain]*event.data['signalerr'])

                if (event.data['status']&0x4) or (event.data['status']&0x10):
                    tgraph = tgraph_deviation_bad[gain]
                elif (event.data['status']&0x1f)!=0:
                    tgraph = tgraph_deviation_affected[gain]
                else:
                    tgraph = tgraph_deviation[gain]
                ...
                if ('f_laser' in event.data):
                    npoints = tgraph_flaser.GetN()
                    tgraph_flaser.SetPoint(npoints, event.run.time_in_seconds-self.time_min, 100.*(1.-event.data['f_laser']))

        self.problemset = set()
        for event in self.HG_evts+self.LG_evts: # fill the histogram
            if 'HV' in event.data:
                npoints = tgraph_hvabs.GetN()
                tgraph_hvabs.SetPoint(npoints, event.run.time_in_seconds-self.time_min, event.data['HV'])
                ...
                try:
                    ...
                    npoints = tgraph_hvcor.GetN()
                    tgraph_hvcor.SetPoint(npoints, event.run.time_in_seconds-self.time_min,  deltagain)
        ...
        self.c1.cd(1)
        tgraph_signal[0].SetMarkerStyle(29)
        tgraph_bad[0].SetMarkerStyle(29)
        tgraph_bad[0].SetMarkerColor(2)
        tgraph_affected[0].SetMarkerStyle(29)
        tgraph_affected[0].SetMarkerColor(800)
        tgraph_saturated[0].SetMarkerStyle(29)
        tgraph_saturated[0].SetMarkerColor(804)
        tgraph_signal[1].SetMarkerStyle(30)
        tgraph_bad[1].SetMarkerStyle(30)
        tgraph_bad[1].SetMarkerColor(2)
        tgraph_affected[1].SetMarkerStyle(30)
        tgraph_affected[1].SetMarkerColor(800)
        tgraph_saturated[1].SetMarkerStyle(30)
        tgraph_saturated[1].SetMarkerColor(804)
        tgraph_hvabs.SetLineColor(4)
        tgraph_hvabs.SetLineWidth(2)
        tgraph_hvcor.SetLineColor(4)
        tgraph_hvcor.SetLineWidth(2)
        tgraph_flaser.SetLineColor(8)

        legend1 = ROOT.TLegend(0.2, 0.62, 0.41, 0.79)
        legend1.AddEntry(tgraph_signal[0], "Low gain (filter 6)", "p")
        legend1.AddEntry(tgraph_signal[1], "high gain (filter 8)", "p")
        legend1.AddEntry(tgraph_bad[1], "bad/masked status", "p")
        legend1.AddEntry(tgraph_affected[1], "affected|noisy status", "p")
        legend1.AddEntry(tgraph_saturated[1], "saturation", "p")
        legend1.AddEntry(tgraph_hvabs, "HV", "L")

        legend1.Draw()

        for tgraph in tgraph_signal + tgraph_bad + tgraph_affected + tgraph_saturated:
            if tgraph.GetN()>0:
                tgraph.Draw("P,same")
        if tgraph_hvabs.GetN()>0:
            tgraph_hvabs.Draw("L,same")
        else:
        ...
        legend2.AddEntry(tgraph_hvcor, "Gain variation computed from HV", "L")
#        legend2.AddEntry(tgraph_flaser, "1./flaser", "L")
        legend2.Draw()

        tgraph_deviation[0].SetMarkerStyle(29)
        tgraph_deviation_bad[0].SetMarkerStyle(29)
        tgraph_deviation_bad[0].SetMarkerColor(2)
        tgraph_deviation_affected[0].SetMarkerStyle(29)
        tgraph_deviation_affected[0].SetMarkerColor(800)

        tgraph_deviation[1].SetMarkerStyle(30)
        tgraph_deviation_bad[1].SetMarkerStyle(30)
        tgraph_deviation_bad[1].SetMarkerColor(2)
        tgraph_deviation_affected[1].SetMarkerStyle(30)
        tgraph_deviation_affected[1].SetMarkerColor(800)


        for tgraph in tgraph_deviation + tgraph_deviation_bad + tgraph_deviation_affected:
            if tgraph.GetN()>0:
                tgraph.Draw("P,same")
        if tgraph_hvcor.GetN()>0:
            tgraph_hvcor.Sort()
            tgraph_hvcor.Draw("L,same")
        else:
            "No entries in tgraph_hvcor"

        if tgraph_flaser.GetN()>0:
            tgraph_flaser.Sort()
#            tgraph_flaser.Draw("L,same")
        else:
        ...
        # for tgraph in tgraph_signal + tgraph_bad + tgraph_affected + \
        #         tgraph_deviation + tgraph_deviation_bad + tgraph_deviation_affected + \
        #         tgraph_saturated + \
        #         [ tgraph_hvabs, tgraph_hvcor ]:
        #     tgraph.Delete()
