...
class get_lowest_laser_signals(GenericWorker):
    "Find signals with lowest average output (in pC)."
    ...
    def ProcessStop(self):
        ...
        for idx in range(self.PMTool.get_index(3,63,47,1)):
            ...
            if n > 1.:
                ...
                if gain == 0:
                    ...
                    for s in self.minsig_lg_idx[::-1]:
                        if s[1] > self.sum_signals[idx] and len(self.minsig_lg_idx) == self.number:
                            self.minsig_lg_idx.remove(self.minsig_lg_idx[::-1][0])
                            self.minsig_lg_idx.append(["%s_LG"%self.PMTool.get_channel_name(part,module,chan),self.sum_signals[idx],self.var_signals[idx],self.bad_signals[idx]])
                            break
                if gain == 1:
                    ...
                    for s in self.minsig_hg_idx[::-1]:
                        if s[1] > self.sum_signals[idx] and len(self.minsig_hg_idx) == self.number:
                            self.minsig_hg_idx.remove(self.minsig_hg_idx[::-1][0])
                            self.minsig_hg_idx.append(["%s_HG"%self.PMTool.get_channel_name(part,module,chan),self.sum_signals[idx],self.var_signals[idx],self.bad_signals[idx]])
                            break
        ...
        self.minsig_lg_idx.sort(key=lambda x: x[1])
        self.minsig_hg_idx.sort(key=lambda x: x[1])
        self.hist_minimal_signals_LG = ROOT.TH2D("minimal LG signals","minimal LG signals",self.number,0,self.number,1000,0.,max(0.001,self.minsig_lg_idx[::-1][0][1]*1.5))
        self.hist_minimal_signals_HG = ROOT.TH2D("minimal HG signals","minimal HG signals",self.number,0,self.number,1000,0.,max(0.001,self.minsig_hg_idx[::-1][0][1]*1.5))
        ...
        self.plot_name = "lowest_%i_laser_signals_%s"%(self.number,self.PMTool.get_partition_name(self.part-1))
