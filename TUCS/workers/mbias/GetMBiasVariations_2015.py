...
class GetMBiasVariations_2015(GenericWorker):
...
#        f.open('Mbias_variation_2015.txt','r')
        ...
        print("reading file")
        with open('Mbias_NewEcells_variation_2015.txt') as f:
            data = f.readlines()
            for line in data:
...
#                print responseinfo
                ...
                channel = int(responseinfo[2][9:11])-1 # extract channel number (actually PMT)
                ...
                if self.cellname == "E1":
                    ...
                    if "EBC" in responseinfo[2] and (module==28 or module==31 or module==33 or module==36 or module==7 or module==23 or module==42 or module==53 or module==6 or module==24 or module==43 or module==52 or module==27 or module==30 or module==34 or module==37):
                        continue
                if abs(float(responseinfo[1])) < 1.:
                    GraphAll.SetPoint(GraphAll.GetN(), float(responseinfo[0]), float(responseinfo[1]))# just add all information together
...
#        atlasLabel.DrawLatex(0.19,0.88, "ATLAS")
        ...
        c.Print("AllPointsVariation_2017_%s.root" % (self.cellname) )
        c.Print("AllPointsVariation_2017_%s.eps" % (self.cellname) )
        c.Print("AllPointsVariation_2017_%s.png" % (self.cellname) )
        c.Print("AllPointsVariation_2017_%s.C" % (self.cellname) )
