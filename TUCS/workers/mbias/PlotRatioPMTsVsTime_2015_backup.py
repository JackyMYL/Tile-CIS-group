...
class PlotRatioPMTsVsTime_2015_backup(GenericWorker):
    "An analysis  worker for MBias"
    
    def __init__(self, modnum=64, detector_region=None):
        ...
        for event in self.eventsList:
            ...
            for r in range(Loops[0], Loops[1]):
                ...
                for m in range(Range[0],Range[1]):
                    ...
                    if m==9 or m==8 or m==41 or m==72 or m==73 or m==103:
                        continue  
                   #build means for each module separately        
                    for i in range(len(PMTCurrent1)):
                        if PMTCurrent1[i][m]!=0 and PMTCurrent1[i][m]>0.1:# and PMTCurrent1[i][m]<50.0: # a bit of cleaning...general for all modules
                   ...
                   #plot for each run and for each module
                    ...
                    meanerrorgaus = gaus.GetParError(1)        
                    ...
                    self.pmtCurrent1.GetXaxis().SetTitle("Ratio PMTCurrent %s/D5 (module %i)" % (self.detector_region, m))
            ...
            #histoMod = TH1F("histoMod","",64,0.5,64.5)
            #for i in range(len(meansgaus)):
                #histoMod->SetBinContent(histoMod->FindBin(moduleNumber[i]), meansgaus[i])
                
            #cMod = TCanvas("cmod",'',500,500)
            #histoMod->Draw()
            #histoMod->GetXaxis()->SetTitle("Module Number")
            #histoMod->GetYaxis()->SetTitle("Average response wrt D5")
                    
            #cMod->Print("%s/E4oD5_Cside.eps" % (self.dir))        
...
#           del ctest2, getmean
            ...
            runtime = datetime.datetime.strptime(event.run.time,'%Y-%m-%d %H:%M:%S')# get runtime
             ...
             # find first run (lowest runNumber, because runs don't get sorted!)        
...
##            Errors.append(profile.GetBinError(profile.FindBin(runNumber[i])))
        ...
        self.RatioPlot.GetYaxis().SetTitle("Current ratio %s/D5" % (self.detector_region))
        
                         
        c.Modified()
        c.Update()
           
        self.plot_name = "TimePlot_MBias_2015"   
        ...
        self.RelativeRatio = ROOT.TGraph(len(Means), Time, relat)
        self.RelativeRatio.Draw("AP")
        self.RelativeRatio.SetMarkerStyle(21)
        self.RelativeRatio.SetMarkerSize(0.4)
        ...
        self.plot_name1 = "RelatVarTime_2015"
        # save plot, several formats...
