...
class GetLumiCoefficient(GenericWorker):
    ...
    def        ProcessStop(self):
        ...
        for event in self.eventsList:
            ...
            print('Run', event.run.runNumber)
            if  event.run.runNumber>220000: continue
        ...
        self.CurrentVsInstLumiA = ROOT.TH2F("CurrentVsInstLumiA",'', 30, minlumi, maxlumi, 30, minimum-0.5,maximum+5.)
        self.CurrentVsInstLumiC = ROOT.TH2F("CurrentVsInstLumiC",'', 30, minlumi, maxlumi, 30, minimum-0.5,maximum+5.)
#        fitplotA = ROOT.TH2F("fitplotA",'', 25, minlumi+2, maxlumi, 70, minimum-0.5,maximum+5.)        
#        fitplotC = ROOT.TH2F("fitplotC",'', 25, minlumi+2, maxlumi, 70, minimum-0.5,maximum+5.)        
                
        #fill both fit plots
        for i in range(len(pairsA)):
            if pairsA[i][1]==0.0: continue
            self.CurrentVsInstLumiA.Fill(pairsA[i][0],pairsA[i][1]) # fill 2d histogram
#            if pairsA[i][0]>=minlumi+2:
...
#                fitplotC.Fill(pairsC[i][0],pairsC[i][1]) # histogram to be fitted later
        ...
        #self.CurrentVsInstLumiA.GetMaximumBin(xMaxA,yMaxA,zMaxA)                                                                                                                                                                                   #self.CurrentVsInstLumiC.GetMaximumBin(xMaxC,yMaxC,zMaxC)                                                                                                                                                                           
        ...
        fitplotA = ROOT.TH2F("fitplotA",'', 25, minlumi+2, maxlumi, 50, minimum-0.5,newMaxCurrent)
        fitplotC = ROOT.TH2F("fitplotC",'', 25, minlumi+2, maxlumi, 50, minimum-0.5,newMaxCurrent)
...
#        ROOT.gPad.SetRightMargin(0.2)
        ...
        self.plot_name = "CurrentVsLumi_MBiasSplit"
        ...
        # don't use the profile, try to get rid of outliers using projections

#        profileA = fitplotA.ProfileX()
        profileA = ROOT.TH1F("profileA","",fitplotA.GetNbinsX(),minlumi+2,maxlumi)
        for bin in range(1,fitplotA.GetNbinsX()+1):
            histoProj = fitplotA.ProjectionY("histoProj",bin,bin)
            histoProj.GetXaxis().SetRange(histoProj.GetMaximumBin()-5,histoProj.GetMaximumBin()+5)
            profileA.SetBinContent(bin,histoProj.GetMean())
            profileA.SetBinError(bin,histoProj.GetRMS())
            del histoProj

#        profileC = fitplotC.ProfileX()
        profileC = ROOT.TH1F("profileC","",fitplotC.GetNbinsX(),minlumi+2,maxlumi)
        for bin in range(1,fitplotC.GetNbinsX()+1):
            ...
            profileC.SetBinError(bin,histoProj.GetRMS())
            del histoProj

        pol1 = ROOT.TF1("pol1","pol1",minlumi+2, maxlumi)
        profileA.Fit('pol1',"r")

        cfitA = ROOT.TCanvas('cfitA','',700,500)
        cfitA.cd()
        ...
        legendA.AddEntry(profileA, "Data 2012, #sqrt{s}=8 TeV", "ep")
        ...
        fitfuncA = ROOT.TH1F("fitfuncA","", 1 ,minlumi+2, maxlumi)
        ...
        ratiohistA = ROOT.TH1F("ratiohistA","",25, minlumi+2, maxlumi)
...
#        profileC = fitplotC.ProfileX()
        ...
        cfitC = ROOT.TCanvas('cfitC','',700,500)
        cfitC.cd()
        pad1 = ROOT.TPad("pad1","pad1",0,0.420,.99,1)
        pad1.SetBottomMargin(0)
        pad1.Draw()
        ...
        legendC.AddEntry(profileC, "Data 2012, #sqrt{s}=8 TeV", "ep")
        ...
        fitfuncC = ROOT.TH1F("fitfuncC","", 1 ,minlumi+2, maxlumi)
        ...
        ratiohistC = ROOT.TH1F("ratiohistC","",25, minlumi+2, maxlumi)
        ...
        det_reg = self.PMTool.getRegionName(self.regionName)
...
33:0.75,37:0.85,40:0.75,39:0.6,44:0.85,46:0.95}
        ...
        f = open('coefficient_vs_eta_data2012_channelsFix.txt', 'a+')
