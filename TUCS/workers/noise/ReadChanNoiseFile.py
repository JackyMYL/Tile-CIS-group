...
class ReadChanNoiseFile(ReadGenericCalibration):
    '''
    Reads channel noise values from calibration ntuple (RawCh_NoiseCalib)
    '''
    ...
    def addTree(self,runNumber):
            f, t = self.getFileTree('RawCh_NoiseCalib_1_%s_%s.root' % (runNumber,'Ped'), 'RawCh_NoiseCalib')
            if [f, t] == [None, None]:
    ...
    def ProcessRegion(self, region):
        ...
        for event in region.GetEvents():
            if event.run.runType == 'Ped':
                newevents.add(event)
                self.addTree(event.run.runNumber)
            if event.run.runType == 'all':
