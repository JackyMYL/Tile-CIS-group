...
class PlotModuleChannel(GenericWorker):
    ...
    def ProcessRegion(self, region):
        ...
        for event in region.GetEvents():
            if event.run.runType == self.runType1 and 'low' in region.GetHash():
                if self.parameter1 in event.data:
        ...
        for event in region.GetEvents():
            if event.run.runType == self.runType2 and 'high' in region.GetHash():
                if self.parameter2 in event.data:
