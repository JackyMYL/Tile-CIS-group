...
#
# October 5, 2012 Adapted to update UPD1 channel status directly by Noah Wasserman <noah.david.wasserman@cern.ch>
...
class WriteChanStatUPD1(ReadGenericCalibration):
    "The Channel Status DB Writer to tileSqlite.UPD1.db"
    
    def __init__(self, tag='RUN2-HLT-UPD1-00', runType='all', run = 0, sqlfn='tileSqlite.UPD1.db'):
        self.tag = tag
    ...
    def ProcessStart(self):
        ...
        if not self.db:
            print("WriteChanStatUPD1: Failed to open a connection to the COOL SQLite3 database")
        else:
            if self.runType == 'L1Calo':
                ...
                self.folderTag = TileCalibTools.getFolderTag('COOLOFL_TILE/CONDBR2', self.folder, self.tag)
    ...
    def ProcessStop(self):
        ...
        if self.db and self.mgr:
            ...
            if self.runType == 'L1Calo':
                ...
                self.mgr.commitToDb(self.db, self.folder, self.folderTag, TileBchDecoder.BitPat_onl01, author, comment, (run,0))
            else:
                self.mgr.commitToDb(self.db, self.folder, self.folderTag, TileBchDecoder.BitPat_ofl01, author, comment, (run,0))
    ...
    def ProcessRegion(self, region):
        ...
        for event in region.GetEvents():
            ...
            if 'gain' in region.GetHash():
                setbit = False
                if 'goodRegion' in event.data and not event.data['goodRegion']:
                    setbit = True
                ...
                if event.run.runType == 'L1Calo':
                    if event.data['isHalfGainL1']:
                        ...
                        self.mgrdelAdcProblem(x, y-z, z, w, TileBchPrbs.TrigHalfGain)
                    
                    if event.data['isNoGainL1']:
                        self.mgr.addAdcProblem(x, y-1, z, w, TilceBchPrbs.TrigNoGain)
                    else:
                    ...
                    if type:
                        if setbit:
                            ...
                            return # this will break things for many run types at the same time
                        else:
                            self.mgr.delAdcProblem(x, y-1, z, w, type)
                            
                    else:
                        print('Unkown runtype ', event.run.runType)
