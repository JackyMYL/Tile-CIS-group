...
class DumpCISUncalibrated(GenericWorker):
    ...
    def ProcessRegion(self, region):
        ...
        for event in region.GetEvents():
            if event.run.runType == self.runType and 'isCalibrated' in event.data:
                if event.data['isCalibrated']:
                    calibrated = True
                foundCalibrated = True

            if event.run.runType == 'Repair' and 'isBad' in event.data:
                if event.data['isBad']:
                    repaired = True
                foundRepair = True

            if event.run.runType == self.runType and 'isStable' in event.data:
        ...
        for event in region.GetEvents():
            if 'scan' in event.data:
                ...
                tl.DrawLatex(xoffset,0.85,"Run %d" % event.run.runNumber)
                tl.DrawLatex(xoffset,0.80,"Slope This Run: %f" % event.data['calibration'])
                tl.DrawLatex(xoffset,0.75,"DB Slope: %f" % event.data['f_cis_db'])
                
                tl.DrawLatex(xoffset,0.70,"DB Variation: %f %s" % (100*(event.data['calibration'] - event.data['f_cis_db'])/event.data['f_cis_db'], '%'))
